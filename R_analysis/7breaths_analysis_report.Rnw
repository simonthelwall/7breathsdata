...
pValFunTex <- function(x){
  if (x < 0.001) z <- as.character("p \textless 0.001")
  else if (x < 0.01) z <- as.character("p \textless 0.01")
  else if (x < 0.05) z <- as.character("p \textless 0.05")
  else if (x < 0.1) z <- as.character("p \textless 0.1")
  else z <- as.character(paste("p = ", round(x, 2), sep = ""))
  return(z)
}
@
...
Data consisted of \Sexpr{length(df2$rr_id)} observations on \Sexpr{sum(df2$uniq, na.rm = TRUE)} individuals. 
...
(\Sexpr{pValFunTex(0.5)}) %(\Sexpr{pValFunTex(c.test.results$p.value)[1]}).
...
<<corr_p_val, echo=TRUE, results='asis'>>=
pValFunTex(c.test.results$p.value)
@
\end{document}